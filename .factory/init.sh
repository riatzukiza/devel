...
DATABASE_URL=postgres://localhost:5432/threat_radar
...
docker exec open-hax-openai-proxy-open-hax-openai-proxy-db-1 \
  psql -U postgres -tc "SELECT 1 FROM pg_database WHERE datname='threat_radar'" | grep -q 1 \
  || docker exec open-hax-openai-proxy-open-hax-openai-proxy-db-1 \
    psql -U postgres -c "CREATE DATABASE threat_radar" 2>/dev/null || true
