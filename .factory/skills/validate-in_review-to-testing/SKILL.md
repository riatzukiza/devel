# Skill: validate-in_review-to-testing

Validates the FSM transition `in_review → testing` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: in_review`.
2. Confirm `testing` is a valid next state from `in_review`.
...
4. If `testing` has WIP caps, verify capacity (via kanban CLI / configured limits).
