# Skill: validate-incoming-to-icebox

Validates the FSM transition `incoming → icebox` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: incoming`.
2. Confirm `icebox` is a valid next state from `incoming`.
