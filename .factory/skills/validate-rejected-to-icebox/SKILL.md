# Skill: validate-rejected-to-icebox

Validates the FSM transition `rejected → icebox` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: rejected`.
2. Confirm `icebox` is a valid next state from `rejected`.
