# Skill: validate-todo-to-in_progress

Validates the FSM transition `todo → in_progress` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: todo`.
2. Confirm `in_progress` is a valid next state from `todo`.
