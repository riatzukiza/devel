# Skill: validate-blocked-to-breakdown

Validates the FSM transition `blocked → breakdown` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: blocked`.
2. Confirm `breakdown` is a valid next state from `blocked`.
