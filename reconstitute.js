...
async function ollamaEmbedOne(E, db, input) {
    const text = input ?? "";
...
async function ollamaChat(E, db, args) {
    const key = `cache:chat:${sha256(JSON.stringify({ model: E.OLLAMA_CHAT_MODEL, messages: args.messages, tools: args.tools, t: args.temperature ?? 0 }))}`;
    const cached = await ttlGet(db, key);
    if (cached)
        return cached;
...
async function recordPath(db, pathStr) {
    const p = normalizePathKey(pathStr);
    if (!p)
        return false;
    const key = `state:recorded_paths`;
    const current = (await db.get(key).catch(() => []));
...
async function listRecordedPaths(db) {
    return (await db.get(`state:recorded_paths`).catch(() => [])).map(normalizePathKey);
}
async function getFileDescription(db, pathStr) {
    const p = normalizePathKey(pathStr);
    return (await db.get(`desc:path:${p}`).catch(() => ""));
}
async function describeFile(db, pathStr, append) {
    const p = normalizePathKey(pathStr);
    const prev = await getFileDescription(db, p);
    const next = prev ? `${prev}\n\n${append}` : append;
    await db.put(`desc:path:${p}`, next);
}
async function listAllDescriptions(db) {
    const out = [];
...
async function addNoteTitle(db, title) {
...
async function listNotesTitles(db) {
    return (await db.get(`notes:index`).catch(() => [])).slice().sort();
}
async function searchSessionsAsContext(E, db, sessionsCol, query, where) {
...
function buildTools(E, db, sessionsCol, notesCol, runState) {
    ...
    const handlers = {
        ...
        async describe_file(args) {
            ...
            if (!p || !text)
                return `{"ok":false,"error":"path/text required"}`;
        ...
        async search_sessions(args) {
            ...
            const E2 = { ...E, SEARCH_LIMIT: Number.isFinite(limit) ? limit : oldLimit, SEARCH_THRESHOLD: threshold };
...
async function runToolLoop(E, db, baseMessages, tools, handlers, maxIters) {
...
function extractPathsLoose(text) {
    ...
    for (const m of text.matchAll(re))
        paths.add(normalizePathKey(m[2]));
    return [...paths];
}
async function exportMarkdownTree(E, db, root, runId) {
    const outRoot = node_path_1.default.resolve(E.OUTPUT_DIR, runId);
    ...
    await promises_1.default.writeFile(node_path_1.default.join(outRoot, "index.md"), indexBody, "utf8");
    for (const d of within) {
        const p = normalizePathKey(d.path);
        const filePath = node_path_1.default.join(outRoot, p + ".md");
...
function makeSystemPrompt(root) {
    return [
...
function makeUserPromptForQuestion(pathStr, question) {
    return question.replaceAll("{path}", normalizePathKey(pathStr));
}
async function reconstitute(E, targetRoot) {
    const root = normalizePathKey(targetRoot);
    const runId = `run_${sha256(root).slice(0, 12)}`;
    await ensureDir(node_path_1.default.dirname(E.LEVEL_DIR));
    ...
    const processed = new Set((await db.get(processedKey).catch(() => [])).map(normalizePathKey));
    const queueArr = (await db.get(queueKey).catch(() => [root])).map(normalizePathKey);
    const queue = uniq(queueArr.filter((p) => isWithinRoot(p, root)));
    const runState = { root };
    ...
    while (queue.length > 0) {
        ...
        for (let qi = 0; qi < DEFAULT_QUESTIONS.length; qi++) {
            ...
            const systemMsg = { role: "system", content: makeSystemPrompt(root) };
            const userMsg = { role: "user", content: question };
            const baseMessages = [systemMsg, ...search.context_messages, userMsg];
