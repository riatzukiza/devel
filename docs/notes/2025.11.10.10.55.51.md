...
* **Index once, update forever**: use file watchers already common in your workflow (mirrors the lsp watcher constraints you set in Emacs) to keep the index fresh【turn1file10】.
* **Plug into Promethean** primitives you’ve already sketched:

  * *Event capture* → route opencode events through your **EventCapturePlugin** as the ingress into memory【turn1file8】【turn1file9】.
  * *Structures/Agents* → align with **Cephalon / Eidolon** as “organs,” and reuse your vector index notion (e.g., `repo-embeddings`)【turn1file18】【turn1file19】.
  * *Data intake* → your `data/` pattern for audio→transcripts is an ingestion precedent you can reuse for other streams【turn1file4】.
  * *Pseudo space* → treat this as living design in `pseudo/` per AGENT.md’s guidance【turn1file14】.
...
* **Human‑first docs:** Keep selected context snapshots as Markdown in `memdocs/` using your **unique doc format** (already configured in `.dir-locals.el`) so filenames are collision‑free timestamps【turn1file10】.
...
> You already model vector corpora (e.g., `repo-embeddings` using Chroma) in `pseudo/pil.hy`; the same pattern works for personal memory【turn1file18】【turn1file19】.
...
**Scoring:** Combine similarity, recency, connectivity, and frequency.

[
\text{score}(m) = \alpha \cdot \text{cos}(q, m) ;+; \beta \cdot \text{sparse}(q, m) ;+; \gamma \cdot \text{conn}(m, C) ;+; \delta \cdot \text{recency}(m) ;+; \epsilon \cdot \text{freq}(m)
]
...
* **EventCapturePlugin**: “Comprehensive event capture and semantic search functionality.” Use it as ingress to the WAL【turn1file8】【turn1file9】.
* **AsyncSubAgentsPlugin**: good place to run background consolidation/summarization without blocking the main loop【turn1file8】.
* **Cephalon** as the organ that “perceives/route/prompt” (fits the blackboard hub role)【turn1file18】.
* **Pandora/Eris**: thinker/prover pattern is already sketched via agents & profiles; fold your role list into profiles and budgets (see `pandora.dev.search.v1`)【turn1file19】.

---

## 9) Indexing projects (bootstrap + continuous)

* **Boot**: parse repo → build symbols (AST), docstrings, simple comments, similarity graph. You already imagine a **vector index over repo** (`repo-embeddings` with Chroma)【turn1file19】.
* **Continuous**: file watchers + commit hooks; mirror your LSP watcher thresholds (you’ve configured `lsp-file-watch-threshold` in Emacs) to avoid thrash【turn1file10】.
...
* **Thinker** (hypothesize) ↔ *Pandora* profile (discover/hypothesize)【turn1file19】
* **Prover** (falsify) ↔ *Eris* adversarial profile (perturb/stress‑test)【turn1file19】
* **Planner/Actor** ↔ Tasks/Messages/Messaging plugins (objectives→actions)【turn1file8】
* **Observer/Sensor** ↔ EventCapturePlugin → Cephalon routing【turn1file8】【turn1file18】
* **Learner/Trainer** ↔ Profiles + telemetry (“learn-from-clicks t”), plus offline consolidation jobs【turn1file19】.
...
* **Eidolon fields**: Treat *Eidolon* as the *semantic field* overlay (feature space + relation weights) backing the associative graph. You already list `Eidolon` as a structure used by *Pandora*【turn1file19】—formalize it as **graph schema + similarity kernels** and store its tunables in `profile`s.
* **Nooi / Daimoi**: If “daimoi” = pluralized “daimon” (personas/spirits), model them as **profiles** + **memory scopes** that bias ranking/expansion differently per persona (you already have `profiles` with blends/budgets)【turn1file19】. “Nooi” can be the **collective** overlay across personas—i.e., an ensemble profile that aggregates clicks/edge reinforcements cross‑persona.
...
* **Single‑command chores**: `mem build`, `mem compact`, `mem verify` (use your bb/JS task runners; mirrors your `scripts/dev.mjs` pattern for multi‑package tasks【turn1file13】).
* **Human snapshots**: auto‑render the compiled context graph → Markdown under `memdocs/` using your unique timestamp filenames【turn1file10】.
...
* **Event capture + async agents**: indexes/plugins you’ve defined for messaging/events/capture provide natural integration points【turn1file8】【turn1file9】.
* **Org/pseudo territory for design**: `pseudo/AGENT.md` explicitly invites speculative code scaffolds【turn1file14】.
* **Structures**: `Cephalon` and `Eidolon` already exist conceptually as “organs”; treat `Cephalon` as router/blackboard and `Eidolon` as the field/graph layer【turn1file18】【turn1file19】.
* **Vector index precedent**: `repo-embeddings` definition (Chroma) shows the pattern to copy for personal memory indices【turn1file19】.
* **Data intake pattern**: `data/README.md` shows your ingestion/deposition practice (e.g., transcripts) that can be generalized to memory streams【turn1file4】.
...
4. **Event ingress**: wrap **EventCapturePlugin** to call `appendMemory`【turn1file8】.
...
8. **Markdown snapshots**: render to `memdocs/` using your timestamp naming【turn1file10】.
9. **Repo bootstrap**: run initial indexer over `packages/`, `services/`, `pseudo/` (use your `repo-embeddings` pattern)【turn1file19】.
10. **Telemetry**: flip `learn-from-clicks` style counters back into `counters.reads`【turn1file19】.
