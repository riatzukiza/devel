...
* **`pm2-clj` is a pass-through** to PM2, only translating DSL → a temp `ecosystem.config.cjs` and then executing PM2.
* **Canonical format**: `.pm2.edn` (legacy `.pm2.clj`/`.pm2.cljs` are deprecated).
* **`clobber` is your workflow CLI** (stubbed now, uses `pm2-clj` under the hood).
* **`package.json` exposes two bins**: `pm2-clj`, `clobber`.
...
(defn main []
  (let [argv (vec (-> js/process .-argv (.slice 2)))]
    ...
    (when (empty? argv)
      ...
      (println "Usage:")
  (println "  pm2-clj <pm2 args...> <ecosystem.pm2.edn> --mode dev")
  (println "  pm2-clj render <ecosystem.pm2.edn> --mode prod")
      (js/process.exit 1))
