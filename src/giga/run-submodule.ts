...
async function main(): Promise<void> {
  ...
  // A gitlink that was never initialized still leaves an empty directory behind,
  // which every detector below reads as "nothing to do" and reports as success.
  if (await isUninitializedGitlink(absSubPath)) {
    console.error(
      `Submodule ${subPath} is a gitlink that is not initialized (no .git ` +
        `entry). Run \`git submodule update --init -- ${subPath}\` before ` +
        `running ${target}.`
    );
    process.exit(1);
  }
...
async function runTypecheck(label: string, dir: string): Promise<boolean> {
  ...
  // Not gated on `pkg`: a TypeScript submodule may ship a tsconfig and no
  // package.json at all, and skipping it would report success without ever
  // invoking a compiler. buildBinaryCommand already falls back to npx when
  // there is no package manager to detect.
...
async function readPackageJson(dir: string): Promise<PackageJson | null> {
  ...
  try {
    ...
    // Returning null here would be read as "no manifest", making the runner
    // skip the target and report success on a submodule whose package.json is
    // truncated or carries conflict markers.
...
async function runTypeScriptCheck(label: string, dir: string, tsconfig: string, manager: NodePackageManager | null): Promise<boolean> {
  ...
  return run(buildBinaryCommand(manager, "tsc", args, "typescript"), dir, { label: `tsc ${label}` });
}

function buildBinaryCommand(
  manager: NodePackageManager | null,
  bin: string,
  args: string[],
  npmPackage: string = bin,
): string[] {
  switch (manager) {
    case "pnpm":
    ...
    case "bun":
      return ["bunx", "--package", npmPackage, bin, ...args];
    case "npm":
    default:
      // Without --package, npm infers the package from the binary name, so
      // `npx --yes tsc` fetches the deprecated `tsc` package (v2.0.4, "A
      // deprecated release of the TypeScript compiler") instead of typescript.
      return ["npx", "--yes", "--package", npmPackage, bin, ...args];
  }
}
...
/**
 * True when `absSubPath` is a gitlink its containing repository knows about but
 * that has never been initialized.
 *
 * Asking git rather than parsing `.gitmodules` is what makes this correct for
 * nested submodules: a gitlink declared in an intermediate submodule's own
 * `.gitmodules` is invisible to the workspace-root file, but `ls-files --stage`
 * reports mode 160000 for it at any depth. Plain tracked directories report
 * their files' own modes instead, so fixtures are unaffected.
 */
async function isUninitializedGitlink(absSubPath: string): Promise<boolean> {
  const proc = spawn({
    cmd: ["git", "ls-files", "--stage", "--", basename(absSubPath)],
    cwd: dirname(absSubPath),
    stdout: "pipe",
    stderr: "pipe",
  });
  ...
  const isGitlink = out.split("\n").some((line) => line.startsWith("160000 "));
  return isGitlink && !(await pathExists(join(absSubPath, ".git")));
}
