...
branches = [
    ('Reopening', state['branches']['reopening']),
    ('Effective closure', state['branches']['effective_closure']),
    ('Wider escalation', state['branches']['wider_escalation']),
]
windows = [
...
for key, value in state['states'].items():
    note = textwrap.shorten(value.get('notes', ''), width=82, placeholder='…')
    confidence = round(float(value.get('confidence', 0.0)) * 100)
    ax.text(780, ystate, f"• {key}: {value['score']}/4 ({value.get('trend','')}, c={confidence}%) — {note}", fontsize=10, ha='left', va='top')
    ystate -= 34
