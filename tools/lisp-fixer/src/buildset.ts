...
function run(cmd: string, args: string[], cwd: string, timeout = 30000): Promise<number> {
...
const ds: string[] = [];
if (exts.has(".clj") || exts.has(".cljs") || exts.has(".cljc")) ds.push("clj");
...
async function main() {
const args = mri(process.argv.slice(2), { string: ["root", "prompt", "out"], default: { prompt: "build", out: "data/buildset.jsonl" } });
...
if (!allowedPrompts.includes(args.prompt)) {
  console.error(`Error: Prompt "${args.prompt}" not in allowlist: ${allowedPrompts.join(", ")}`);
  process.exit(1);
}

const roots = await fg([`${args.root}/**/.git`], { onlyDirectories: true, deep: 3 });
const out = createWriteStream(args.out, { flags: "w" });

for (const gitdir of roots) {
const repoRoot = path.dirname(gitdir);
const files = await fg(["**/*.{clj,cljs,cljc,lisp,lsp,el,scm,rkt}"], { cwd: repoRoot, dot: true });
...
process.stdout.write(pc.dim(`probing ${repoRoot}… `));
    try {
      ...
      if (status === "green") {
        out.write(JSON.stringify({ repo: path.basename(repoRoot), root: repoRoot, prompt: args.prompt, status, dialects }) + "\n");
      }
    } catch (error) {
