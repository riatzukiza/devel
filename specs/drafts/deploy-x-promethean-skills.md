...
- `.opencode/skill/<name>/SKILL.md` links exist for each new skill.
- Workspace `AGENTS.md` tells future agents that `Deploy X` means the full Promethean deploy bootstrap flow.
- A workspace command/template exists at `.opencode/commands/deploy-promethean-service.md`.
...
- Linked `promethean-service-deploy`, `promethean-host-slotting`, and `pr-promotion-workflows` into `.opencode/skill/` for workspace-visible discovery.
...
- Updated `AGENTS.md` so future agents interpret `Deploy X` using this convention.
- Added `.opencode/commands/deploy-promethean-service.md` as a workspace-visible command/template that restates the contract and required skills/env.
...
- `.opencode/commands/deploy-promethean-service.md` exists and matches the deployment contract ✅
