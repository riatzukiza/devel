...
const createMockClient = () => ({
    session: {
        ...
        get: async ({ path: sessionPath }) => ({
            data: {
                id: sessionPath.id,
                title: `Mock Session ${sessionPath.id}`,
            },
        }),
        delete: async ({ path: sessionPath }) => {
            console.log(`Mock: Deleted session ${sessionPath.id}`);
        },
        messages: async ({ path: sessionPath }) => {
        ...
        prompt: async ({ path: sessionPath, body, }) => {
            console.log(`Mock: Sent prompt to session ${sessionPath.id}:`, body.parts[0]?.text?.substring(0, 50) + '...');
        },
    },
    event: {
        subscribe: async () => ({
        ...
        switch (command) {
            case 'spawn': {
                ...
                const delegates = flags.delegates
                    ? flags.delegates.split(',').map((d) => d.trim())
                    : undefined;
            ...
            case 'monitor': {
                ...
                if (summary.agents.length > 0) {
                    ...
                    summary.agents.forEach((agent, index) => {
                        ...
                        console.log(`   Task: ${agent.task.substring(0, 80)}${agent.task.length > 80 ? '...' : ''}`);
            ...
            case 'list': {
                ...
                console.log('\n📚 Sessions');
                console.log('============');
                console.log(`Total: ${sessions.totalCount} | Page ${sessions.pagination.currentPage}/${sessions.pagination.totalPages}`);
                console.log(`Active: ${sessions.summary.active} | Waiting: ${sessions.summary.waiting_for_input} | Idle: ${sessions.summary.idle} | Agent Tasks: ${sessions.summary.agentTasks}`);
                if (sessions.sessions.length > 0) {
                    console.log('\n📋 Session Details:');
                    sessions.sessions.forEach((session, index) => {
                        const agentMarker = session.isAgentTask ? '🤖' : '💬';
                        console.log(`${index + 1}. ${agentMarker} ${session.id.substring(0, 8)}... (${session.activityStatus})`);
            ...
            case 'send': {
                ...
                const result = await orchestrator.sendAgentMessage(sessionId, message, priority, messageType);
                console.log(result);
                break;
            }
            case 'cleanup': {
