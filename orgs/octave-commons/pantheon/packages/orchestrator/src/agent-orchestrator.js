...
export class AgentOrchestrator {
    ...
    async spawnAgent(options) {
        ...
        try {
            ...
            console.log(`🚀 Spawned new sub-agent "${agentName}" (session: ${subSession.id}) with task: ${prompt}`);
            // Prepare message parts
            const parts = [{ type: 'text', text: taskDescription }];
            ...
            if (this.config.persistenceEnabled && this.agentTaskStore) {
                ...
                for (const task of storedTasks) {
                    if (!allAgentTasks.has(task.id || '')) {
                        const sessionId = task.metadata?.sessionId || task.id || 'unknown';
                        const startTime = typeof task.timestamp === 'number'
                            ? task.timestamp
                            : task.timestamp
                                ? new Date(task.timestamp).getTime()
                                : Date.now();
                        const status = task.metadata?.status || 'idle';
                        const lastActivity = typeof task.metadata?.lastActivity === 'number'
                            ? task.metadata.lastActivity
                            : task.metadata?.lastActivity
                                ? new Date(task.metadata?.lastActivity).getTime()
                                : startTime;
                        const completionMessage = task.metadata?.completionMessage;
    ...
    async getAgentStatus(sessionId) {
        try {
            ...
            if (!task && this.config.persistenceEnabled && this.agentTaskStore) {
                ...
                if (storedTask) {
                    task = {
                        sessionId: storedTask.metadata?.sessionId || storedTask.id || 'unknown',
                        task: storedTask.text,
                        startTime: typeof storedTask.timestamp === 'number'
                            ? storedTask.timestamp
                            : new Date(storedTask.timestamp).getTime(),
                        status: storedTask.metadata?.status || 'idle',
                        lastActivity: typeof storedTask.metadata?.lastActivity === 'number'
                            ? storedTask.metadata?.lastActivity
                            : new Date(storedTask.metadata?.lastActivity ||
                                storedTask.timestamp).getTime(),
                        completionMessage: storedTask.metadata?.completionMessage,
                    };
                }
            }
            if (!task) {
    ...
    async sendAgentMessage(sessionId, message, priority = 'medium', messageType = 'instruction') {
        try {
            ...
            const safeSenderId = senderSessionId.length > 8 ? senderSessionId.substring(0, 8) : senderSessionId;
...
*This is an inter-agent communication sent via the AgentOrchestrator. Please respond if this requires action or acknowledgment.*`;
        ...
        try {
            ...
            const enhancedSessions = await Promise.all(paginatedSessions.map(async (session) => {
                try {
                    ...
                    return {
                        ...
                        lastActivityTime: new Date().toISOString(),
                        sessionAge,
                        activityStatus: agentTask?.status === 'running'
                            ? 'waiting_for_input'
                            : 'idle',
                        isAgentTask: !!agentTask,
                        agentTaskStatus: agentTask?.status,
            ...
            return {
                ...
                summary: {
                    ...
                    waiting_for_input: enhancedSessions.filter((s) => s.activityStatus === 'waiting_for_input').length,
    ...
    async cleanupCompletedAgents(olderThanMinutes = 60) {
        const now = Date.now();
    ...
    async updateAgentTaskStatus(sessionId, status, completionMessage) {
    ...
    monitorAgentTasks() {
        ...
        for (const [sessionId, task] of Array.from(this.agentTasks.entries())) {
            if (task.status === 'running' && now - task.lastActivity > this.config.timeoutThreshold) {
                console.warn(`⚠️ Agent task timeout for session ${sessionId} (inactive for ${this.config.timeoutThreshold / 60000} minutes)`);
                this.updateAgentTaskStatus(sessionId, 'failed', 'Task timed out due to inactivity');
            }
        }
    }
    startMonitoring() {
        this.monitoringInterval = setInterval(() => this.monitorAgentTasks(), this.config.monitoringInterval);
    }
    stopMonitoring() {
