...
import { parseMarkdownWorkflows, resolveWorkflowDefinitions, createAgentWorkflowGraph, } from "../index.js";
const MARKDOWN = `# Demo
...
test("resolveWorkflowDefinitions loads external definitions correctly", async (t) => {
    ...
    await fs.writeFile(helperPath, JSON.stringify({ instructions: "Assist with tasks", model: { provider: "openai", name: "gpt-4o-mini" } }, null, 2), "utf8");
    const document = parseMarkdownWorkflows(MARKDOWN, {});
    const workflows = await resolveWorkflowDefinitions(document, { baseDir: tmpDir });
    t.true(workflows.length > 0);
    const workflow = workflows[0];
    t.like(workflow.nodes.find((n) => n.id === "helper")?.definition, { instructions: "Assist with tasks" });
});
...
test("createAgentWorkflowGraph produces correct graph config", async (t) => {
    ...
    const graph = await createAgentWorkflowGraph(workflow, {
