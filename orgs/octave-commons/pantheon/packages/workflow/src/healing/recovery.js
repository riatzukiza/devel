 ...
 */
export class DefaultRecoveryManager {
    activeRecoveries = new Map();
    recoveryHistory = new Map();
    rollbackStack = new Map();
    async executeRecovery(workflow, issue, strategy) {
    ...
    async rollbackRecovery(recoveryId) {
        const recovery = this.activeRecoveries.get(recoveryId) || this.findRecoveryInHistory(recoveryId);
    ...
    async listRecoveries(workflowId) {
        return this.recoveryHistory.get(workflowId) || [];
    }
    // Private methods
    async executeStrategy(workflow, issue, strategy) {
    ...
    async executeRestartStrategy(workflow, issue, strategy) {
        const startTime = Date.now();
        const changes = [];
        const sideEffects = [];
        try {
    ...
    async executeScaleStrategy(workflow, issue, strategy) {
        const startTime = Date.now();
        const scaleFactor = strategy.parameters.find((p) => p.name === 'scaleFactor')?.defaultValue || 1.5;
        const changes = [];
        const sideEffects = [];
        try {
    ...
    async executeReconfigureStrategy(workflow, issue, strategy) {
        const startTime = Date.now();
        try {
    ...
    async takeSnapshot(workflowId, recoveryId) {
        const snapshot = {
    ...
    findRecoveryInHistory(recoveryId) {
