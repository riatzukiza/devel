 ...
 */
export class DefaultWorkflowMonitor {
    config;
    ...
    async stopMonitoring(workflowId) {
    ...
    async detectAnomalies(workflowId) {
        const history = this.metricsHistory.get(workflowId);
    ...
    async setupAlerts(workflowId, config) {
    ...
    async checkAlerts(workflowId) {
        const history = this.metricsHistory.get(workflowId);
        const rules = this.alertRules.get(workflowId);
        if (!history || !rules || history.length === 0)
            return;
    ...
    evaluateCondition(metrics, condition) {
        let value;
    ...
    async triggerAlert(workflowId, condition, metrics) {
        const alert = {
    ...
    async sendHealthCheck(workflowId) {
    ...
    detectPerformanceDegradation(workflowId, metrics) {
        if (metrics.length < 5)
            return null;
    ...
    detectResourceExhaustion(workflowId, metrics) {
    ...
    detectIncreasingErrors(workflowId, metrics) {
    ...
    detectMemoryLeak(workflowId, metrics) {
