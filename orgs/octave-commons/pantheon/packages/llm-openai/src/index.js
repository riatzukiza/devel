    ...
    for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
        try {
            ...
            if (attempt === config.maxRetries || !isRetryable) {
                throw new OpenAIAdapterError(`OpenAI operation failed after ${attempt + 1} attempts: ${lastError.message}`, lastError, isRetryable);
            }
            ...
            console.warn(`OpenAI operation failed (attempt ${attempt + 1}/${config.maxRetries + 1}), retrying in ${finalDelay}ms:`, lastError.message);
...
export const makeOpenAIAdapter = (config) => {
    ...
    return {
        complete: async (messages, opts) => {
            ...
            const frequencyPenalty = validatedOpts.frequencyPenalty ?? validatedConfig.defaultFrequencyPenalty;
            const presencePenalty = validatedOpts.presencePenalty ?? validatedConfig.defaultPresencePenalty;
            ...
            if (frequencyPenalty !== undefined)
                completionParams.frequency_penalty = frequencyPenalty;
            if (presencePenalty !== undefined)
                completionParams.presence_penalty = presencePenalty;
            return retryWithBackoff(async () => {
                try {
                    ...
                    const isRetryable = isRetryableError(error instanceof Error ? error : new Error(errorMessage), retryConfig.retryableErrors);
                    throw new OpenAIAdapterError(`OpenAI API error: ${errorMessage}`, error instanceof Error ? error : new Error(errorMessage), isRetryable);
                }
            }, retryConfig);
        },
    };
