...
const createMockOpenAI = (responses, shouldFail = false, errorType) => ({
    chat: {
        completions: {
            create: async () => {
                if (shouldFail) {
...
test('OpenAI adapter retries on rate limit errors', async (t) => {
    let attemptCount = 0;
    const mockClient = createMockOpenAI([{ choices: [{ message: { content: 'Success' } }] }], true, 'rate_limit_exceeded');
...
test('OpenAI adapter succeeds after retries', async (t) => {
    let attemptCount = 0;
    const mockClient = createMockOpenAI([{ choices: [{ message: { content: 'Success' } }] }], true, // Fail first attempt
    'temporary_error');
    const adapter = makeOpenAIAdapter({
        apiKey: 'test-key',
        retryConfig: { maxRetries: 2, baseDelay: 10 },
...
test('OpenAI adapter exponential backoff increases delay', async (t) => {
    ...
    const mockClient = createMockOpenAI([{ choices: [{ message: { content: 'Success' } }] }], true, // Fail first 2 attempts
    'temporary_error');
    const adapter = makeOpenAIAdapter({
        apiKey: 'test-key',
