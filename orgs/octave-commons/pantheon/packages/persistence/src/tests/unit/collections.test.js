...
import { validContextSources, emptyContextSources, mixedContextSources, } from '../fixtures/test-data.js';
...
test('should handle managers with missing name property', async (t) => {
    ...
    delete incompleteManager.name;
    const deps = {
        getStoreManagers: async () => [incompleteManager],
    };
    const adapter = makePantheonPersistenceAdapter(deps);
