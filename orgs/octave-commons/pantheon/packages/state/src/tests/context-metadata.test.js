...
test.serial('ContextMetadataService: should handle metadata expiration', async (t) => {
    ...
    await metadataService.setMetadata(agentId, key, { temp: true }, {
        expiresAt: pastDate,
    });
    // Set metadata with expiration in the future
    const futureDate = new Date(Date.now() + 10000); // 10 seconds from now
    await metadataService.setMetadata(agentId, 'future-data', { future: true }, {
...
test.serial('ContextMetadataService: should handle visibility levels', async (t) => {
    ...
    await metadataService.setMetadata(agentId, 'private-data', { secret: 'shhh' }, {
        visibility: 'private',
    });
    await metadataService.setMetadata(agentId, 'shared-data', { team: 'dev' }, {
        visibility: 'shared',
    });
    await metadataService.setMetadata(agentId, 'public-data', { status: 'online' }, {
        visibility: 'public',
    });
...
//# sourceMappingURL=context-metadata.test.js.map