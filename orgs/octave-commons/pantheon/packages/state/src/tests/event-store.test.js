...
test.serial('MockEventStore: should retrieve events by agent ID', async (t) => {
    ...
    t.true(agentEvents.some((e) => e.type === 'event-1'));
    t.true(agentEvents.some((e) => e.type === 'event-2'));
    t.false(agentEvents.some((e) => e.type === 'event-3'));
});
test.serial('MockEventStore: should retrieve events from specific version', async (t) => {
    ...
    t.true(eventsFromV2.some((e) => e.data.version === 2));
    t.true(eventsFromV2.some((e) => e.data.version === 3));
    t.false(eventsFromV2.some((e) => e.data.version === 1));
});
...
test.serial('MockEventStore: should handle concurrent event appends', async (t) => {
    const mockStore = new MockEventStore();
    const agentId = 'agent-123';
    const events = Array.from({ length: 10 }, (_, i) => createMockEvent({ agentId, type: `event-${i}` }));
    ...
    events.forEach((event) => {
        t.true(retrievedEvents.some((e) => e.id === event.id));
    });
});
test.serial('MockEventStore: should maintain event order', async (t) => {
    ...
    const events = Array.from({ length: 5 }, (_, i) => createMockEvent({
        agentId,
