...
test.serial('ContextSharingService: should check share access permissions', async (t) => {
    ...
    t.false(await sharingService.checkShareAccess(targetAgentId, readShare.contextSnapshotId, 'write'));
    t.false(await sharingService.checkShareAccess(targetAgentId, readShare.contextSnapshotId, 'admin'));
    // Check access permissions for write share
    t.true(await sharingService.checkShareAccess(targetAgentId, writeShare.contextSnapshotId, 'read'));
    t.true(await sharingService.checkShareAccess(targetAgentId, writeShare.contextSnapshotId, 'write'));
    t.false(await sharingService.checkShareAccess(targetAgentId, writeShare.contextSnapshotId, 'admin'));
    // Check access permissions for admin share
    t.true(await sharingService.checkShareAccess(targetAgentId, adminShare.contextSnapshotId, 'read'));
    t.true(await sharingService.checkShareAccess(targetAgentId, adminShare.contextSnapshotId, 'write'));
    t.true(await sharingService.checkShareAccess(targetAgentId, adminShare.contextSnapshotId, 'admin'));
});
...
//# sourceMappingURL=context-sharing.test.js.map