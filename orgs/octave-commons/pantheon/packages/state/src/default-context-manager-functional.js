...
import { checkRateLimit, buildContextFromSnapshot, buildContextFromEvents, logSecurityError, } from './context-manager-helpers-functional.js';
// Create initial context manager state
export const createContextManagerState = (eventStore, snapshotStore, snapshotInterval = 100) => ({
...
export const getContext = async (state, agentId) => {
    try {
...
export const updateContext = async (state, agentId, updates) => {
    try {
        const validatedAgentId = SecurityValidator.validateAgentId(agentId);
        ...
        const { nextVersion, newState: versionState } = getNextVersion(state, validatedAgentId, currentContext.version);
        const updatedContext = buildUpdatedContext(currentContext, validatedAgentId, sanitizedUpdates, nextVersion);
        await createUpdateEvent(state.eventStore, validatedAgentId, sanitizedUpdates, currentContext.version, updatedContext.version);
...
export const getNextVersion = (state, agentId, currentVersion) => {
...
export const buildUpdatedContext = (currentContext, agentId, updates, version) => {
    return {
        ...currentContext,
        ...updates,
...
// Create update event
export const createUpdateEvent = async (eventStore, agentId, updates, previousVersion, newVersion) => {
    const event = {
...
export const appendEventToStore = async (eventStore, event) => {
    // Validate and sanitize input
...
export const appendEvent = async (state, event) => {
    try {
        // Validate and sanitize input
        const validatedAgentId = SecurityValidator.validateAgentId(event.agentId);
        const validatedData = SecurityValidator.validateEventData(event.data);
...
// Create snapshot
export const createSnapshot = async (state, agentId) => {
    try {
        // Validate and sanitize input
        const validatedAgentId = SecurityValidator.validateAgentId(agentId);
        ...
        const snapshot = {
            ...
            agentId: validatedAgentId,
            timestamp: new Date(),
            state: SecurityValidator.sanitizeObject(context.state),
...
export const restoreFromSnapshot = async (state, snapshotId) => {
    try {
        // Validate and sanitize input
        ...
        return {
            ...
            state: SecurityValidator.sanitizeObject(snapshot.state),
...
// Delete context
export const deleteContext = async (state, agentId) => {
    try {
        // Validate and sanitize input
        const validatedAgentId = SecurityValidator.validateAgentId(agentId);
        SecurityLogger.log({
...
// Get context history
export const getContextHistory = async (state, agentId, limit) => {
    try {
        // Validate and sanitize input
        const validatedAgentId = SecurityValidator.validateAgentId(agentId);
...
//# sourceMappingURL=default-context-manager-functional.js.map