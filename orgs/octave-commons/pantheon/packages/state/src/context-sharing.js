...
export class ContextSharingService {
    ...
    snapshotStore;
    rateLimiter;
    constructor(shareStore, snapshotStore) {
        this.shareStore = shareStore;
        this.snapshotStore = snapshotStore;
        this.rateLimiter = RateLimiter.getInstance('context-sharing', 60000, 50);
    }
    async shareContext(sourceAgentId, targetAgentId, shareType = 'read', options = {}) {
        try {
            const { sourceId, targetId, type } = await ContextSharingHelpers.validateShareInputs(sourceAgentId, targetAgentId, shareType);
            ...
            const sanitizedPermissions = options.permissions
                ...
                : {};
            const share = ContextSharingHelpers.createShareRecord(sourceId, targetId, type, latestSnapshot.id, {
    ...
    async getLatestSnapshotForAgent(agentId) {
    ...
    async getSharedContexts(agentId) {
        try {
            ...
            await ContextSharingHelpers.checkRateLimit(this.rateLimiter, validatedAgentId, 'getSharedContexts');
    ...
    async batchFetchSnapshots(shares) {
    ...
    combineSharesWithSnapshots(shares, snapshotMap) {
    ...
    async revokeShare(shareId, requestingAgentId) {
        try {
    ...
    async findShareForAgent(shareId, agentId) {
    ...
    async updateSharePermissions(shareId, updates, requestingAgentId) {
        try {
            const validatedShareId = SecurityValidator.validateSnapshotId(shareId);
            const validatedAgentId = SecurityValidator.validateAgentId(requestingAgentId);
            await ContextSharingHelpers.checkRateLimit(this.rateLimiter, validatedAgentId, 'updateSharePermissions');
            ...
            const result = await this.shareStore.updateShare(validatedShareId, sanitizedUpdates);
            SecurityLogger.log({
                type: 'authorization',
                severity: 'low',
                agentId: validatedAgentId,
    ...
    async checkShareAccess(agentId, snapshotId, requiredPermission = 'read') {
        try {
            const validatedAgentId = SecurityValidator.validateAgentId(agentId);
            ...
            await ContextSharingHelpers.checkRateLimit(this.rateLimiter, validatedAgentId, 'checkShareAccess');
            const sharesForSnapshot = await this.getSharesForSnapshot(validatedAgentId, validatedSnapshotId);
    ...
    async getSharesForSnapshot(agentId, snapshotId) {
    ...
    checkPermissionLevel(shares, requiredPermission) {
    ...
    async getShareStatistics(agentId) {
        try {
...
//# sourceMappingURL=context-sharing.js.map