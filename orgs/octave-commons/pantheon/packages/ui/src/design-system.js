...
export function applyAgentDesignTokens(root = document) {
    const el = root.documentElement ? root.documentElement : root;
    if (!el || !el.style)
        return;
    Object.entries(agentDesignTokens).forEach(([k, v]) => {
        const kebabKey = k.replace(/[A-Z]/g, (m) => '-' + m.toLowerCase());
        el.style.setProperty(`--agent-${kebabKey}`, String(v));
    });
}
export function getStatusColor(status) {
    return (agentStatusConfig[status]?.color ||
        agentDesignTokens.statusIdle);
}
export function getTypeColor(type) {
    return (agentTypeConfig[type]?.color || agentDesignTokens.typeGeneral);
}
export function getPriorityColor(priority) {
    ...
    return colors[priority] || agentDesignTokens.priorityMedium;
}
