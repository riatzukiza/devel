...
import { makeOrchestrator, makeInMemoryContextAdapter, makeInMemoryToolAdapter, makeInMemoryLlmAdapter, makeInMemoryMessageBusAdapter, makeInMemorySchedulerAdapter, makeInMemoryActorStateAdapter, } from '@promethean-os/pantheon-core';
...
program
    ...
    try {
        ...
        await orchestrator.tickActor(actor, options.message ? { userMessage: options.message } : undefined);
...
program
    ...
    try {
        ...
        console.log(`Starting actor loop for ${actor.script.name} (${actorId}) with ${interval}ms interval`);
...
program
    ...
    .action(async (toolName, argsString) => {
    try {
        const { adapters } = getSystem();
        let args;
        try {
...
program
    ...
    try {
        ...
        const actor = await adapters.actorState.spawn(llmScript, 'Help users understand the Pantheon framework');
        console.log(`   Created actor: ${actor.id}`);
        // Tick the actor
        console.log('\\n2. Ticking actor...');
