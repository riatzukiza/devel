#!/usr/bin/env node
/**
 * Quartz Garden — Import Graph Builder
 *
 * Generates:
 * - full file→file import graph (DOT + SVG + PNG + JSON)
 * - subsystem/directory summary graph (DOT + SVG + PNG + JSON)
 * - entrypoint-reachable subgraphs for configured entrypoints
 *
 * Notes
 * - This intentionally resolves only *relative* imports (./ and ../).
 * - It also handles the common TS pattern of importing ".js" specifiers that
 *   map back to ".ts" sources.
 */
...
const DEFAULT_OUT_DIR = "services/riatzukiza-portfolio/.quartz-out";
