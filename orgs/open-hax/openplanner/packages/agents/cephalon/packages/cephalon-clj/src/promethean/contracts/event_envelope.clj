...
(def ^:private boundary->internal-type-map
  {"discord.message.created" :discord/message-created
   ...
   "tool.call" :tool/call
   "tool.result" :tool/result
   "llm.assistant.message" :cephalon/thought
   "system.tick" :system/tick
   "admin.command" :admin/command})
   ...
   (let [event-type (internal-type->boundary (:event/type evt))
     ...
     (clean-map
      {:schemaVersion +schema-version+
       :id (or (:event/id evt) (ids/uuid))
       :type event-type
       :timestamp (or (:event/ts evt) (System/currentTimeMillis))
