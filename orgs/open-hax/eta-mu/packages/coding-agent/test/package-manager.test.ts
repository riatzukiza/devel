...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.ETA_MU_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.ETA_MU_OFFLINE = previousOfflineEnv;
		}
...
Content`,
		...
		it("should auto-discover root markdown skills from .ημ skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .ημ", async () => {
			const extDir = join(tempDir, ".ημ", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
			...
			mkdirSync(join(tempDir, ".ημ"), { recursive: true });
			...
			symlinkSync(sharedExtensionsDir, join(tempDir, ".ημ", "extensions"), "dir");
			symlinkSync(sharedSkillsDir, join(tempDir, ".ημ", "skills"), "dir");
			symlinkSync(sharedPromptsDir, join(tempDir, ".ημ", "prompts"), "dir");
			symlinkSync(sharedThemesDir, join(tempDir, ".ημ", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".ημ", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".ημ", "agent");
		...
		it("should dedupe user skill entries when ~/.ημ/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .ημ auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".ημ\n");

			const skillDir = join(tempDir, ".ημ", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".ημ", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .ημ settings base", () => {
			...
			const rel = relative(join(tempDir, ".ημ"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".ημ", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".ημ", "npm")],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".ημ", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".ημ", "npm", "node_modules", "project-current");
			...
			expect(runCommandSpy).toHaveBeenNthCalledWith(
				2,
				"npm",
				["install", "project-old@latest", "project-missing@latest", "--prefix", join(tempDir, ".ημ", "npm")],
				undefined,
			);
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".ημ", "npm", "node_modules", "example");
