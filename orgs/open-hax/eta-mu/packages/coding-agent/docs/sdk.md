...
```bash
npm install @open-hax/eta-mu-cli
```
...
```typescript
import { createAgentSession } from "@open-hax/eta-mu-cli";
...
The `Agent` class (from `@open-hax/eta-mu-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
import { AuthStorage, ModelRegistry } from "@open-hax/eta-mu-cli";
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@open-hax/eta-mu-cli";
...
import { createAgentSession, defineTool } from "@open-hax/eta-mu-cli";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@open-hax/eta-mu-cli";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@open-hax/eta-mu-cli";
