...
```typescript
import type { AutocompleteItem } from "@open-hax/eta-mu-tui";
...
**Important:** Use `StringEnum` from `@open-hax/eta-mu-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { Text, Component } from "@open-hax/eta-mu-tui";
