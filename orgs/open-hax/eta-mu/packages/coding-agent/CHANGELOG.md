...
- Fixed Windows pnpm global install detection to recognize `\\.pnpm\\` store paths, so update notices now suggest `pnpm install -g @open-hax/eta-mu-cli` instead of falling back to npm ([#3378](https://github.com/badlogic/pi-mono/issues/3378))
- Fixed missing `@sinclair/typebox` runtime dependency in `@open-hax/eta-mu-cli`, so strict pnpm installs no longer fail with `ERR_MODULE_NOT_FOUND` when starting `pi` ([#3434](https://github.com/badlogic/pi-mono/issues/3434))
...
- Fixed missing root exports for `RpcClient` and RPC protocol types from `@open-hax/eta-mu-cli`, so ESM consumers can import them from the main package entrypoint ([#3275](https://github.com/badlogic/pi-mono/issues/3275))
...
- Fixed monorepo `tsconfig.json` path mappings to resolve `@open-hax/eta-mu-ai` subpath exports to source files in development checkouts ([#2625](https://github.com/badlogic/pi-mono/pull/2625) by [@ferologics](https://github.com/ferologics))
...
- Added missing `ajv` direct dependency; previously relied on transitive install via `@open-hax/eta-mu-ai` which broke standalone installs ([#2252](https://github.com/badlogic/pi-mono/issues/2252))
...
- Built-in tools now work like custom tools in extensions. To get built-in tool definitions, import `readToolDefinition` / `createReadToolDefinition()` and the equivalent `bash`, `edit`, `write`, `grep`, `find`, and `ls` exports from `@open-hax/eta-mu-cli`.
...
- Added `ToolCallEventResult` to the `@open-hax/eta-mu-cli` top-level and core extension exports so extension authors can type explicit `tool_call` handler return values ([#2458](https://github.com/badlogic/pi-mono/issues/2458))
...
- Faster startup by lazy-loading `@open-hax/eta-mu-ai` provider SDKs on first use instead of import time ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Lazy-load built-in `@open-hax/eta-mu-ai` provider modules and root provider wrappers so coding-agent startup no longer eagerly loads provider SDKs before first use ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Fixed `@open-hax/eta-mu-ai/oauth` extension imports in published installs by resolving the subpath directly from built `dist` files instead of package-root wrapper shims ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level `@open-hax/eta-mu-ai` entry. OAuth login and refresh must be imported from `@open-hax/eta-mu-ai/oauth` ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed extension loading with the new `@open-hax/eta-mu-ai/oauth` export path by aliasing the oauth subpath in the extension loader and development path mapping ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed browser bundling compatibility for `@open-hax/eta-mu-ai` by removing Node-only side effects from default browser import paths ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
## [0.55.4] - 2026-03-02
...
- Extensions now load correctly in compiled Bun binary using `@mariozechner/jiti` fork with `virtualModules` support. Bundled packages (`@sinclair/typebox`, `@open-hax/eta-mu-tui`, `@open-hax/eta-mu-ai`, `@open-hax/eta-mu-cli`) are accessible to extensions without filesystem node_modules.
...
```typescript
// Before (hook)
import type { HookAPI, HookContext } from "@open-hax/eta-mu-cli";
export default function (pi: HookAPI) { ... }

// Before (custom tool)
import type { CustomToolFactory } from "@open-hax/eta-mu-cli";
...
- **Key detection functions removed from `@open-hax/eta-mu-tui`**: All `isXxx()` key detection functions (`isEnter()`, `isEscape()`, `isCtrlC()`, etc.) have been removed. Use `matchesKey(data, keyId)` instead (e.g., `matchesKey(data, "enter")`, `matchesKey(data, "ctrl+c")`). This affects hooks and custom tools that use `ctx.ui.custom()` with keyboard input handling. ([#405](https://github.com/badlogic/pi-mono/pull/405))
...
- `Attachment` type removed. Use `ImageContent` from `@open-hax/eta-mu-ai` instead. Add images directly to message content arrays.
...
- **SDK API changes** ([#296](https://github.com/badlogic/pi-mono/issues/296)):
  ...
  - Use `getModel()` from `@open-hax/eta-mu-ai` for built-in models, `modelRegistry.find()` for custom models + built-in models
...
- **OAuth and model config exports**: Scripts using `AgentSession` directly can now import `getAvailableModels`, `getApiKeyForModel`, `findModel`, `login`, `logout`, and `getOAuthProviders` from `@open-hax/eta-mu-cli` to reuse OAuth token storage and model resolution. ([#245](https://github.com/badlogic/pi-mono/issues/245))
...
- **`getMarkdownTheme()` export**: Custom tools can now import `getMarkdownTheme()` from `@open-hax/eta-mu-cli` to use the same markdown styling as the main UI.
...
- Hooks loader now supports same import aliases as custom tools (`@sinclair/typebox`, `@open-hax/eta-mu-ai`, `@open-hax/eta-mu-tui`, `@open-hax/eta-mu-cli`).
...
- Fixed custom tools failing to load from `~/.pi/agent/tools/` when pi is installed globally. Module imports (`@sinclair/typebox`, `@open-hax/eta-mu-tui`, `@open-hax/eta-mu-ai`) are now resolved via aliases.
...
- Updated `@open-hax/eta-mu-ai` with X-Initiator header support for GitHub Copilot, ensuring agent calls are not deducted from quota. ([#200](https://github.com/badlogic/pi-mono/pull/200) by [@kim0](https://github.com/kim0))
...
- Updated `@open-hax/eta-mu-ai` with interleaved thinking enabled by default for Anthropic Claude 4 models.
...
- Updated `@open-hax/eta-mu-ai` with interleaved thinking support for Anthropic models.
