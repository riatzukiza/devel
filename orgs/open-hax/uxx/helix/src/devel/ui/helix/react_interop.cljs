(ns devel.ui.helix.react-interop
  "React-backed parity layer for the Helix package.

   These wrappers reuse the canonical React implementation from ../dist,
   while accepting idiomatic ClojureScript props.

   Performance notes:
   - callback wrappers are cached by function identity
   - converted CLJS maps/vectors are cached by object identity
   - already-native JS props objects pass through unchanged"
