...
func TestRenderOutcomeText(t *testing.T) {
	b := &Bot{}
	run := RunStatus{Status: "failed", Error: "boom", Trace: map[string]any{"tool_execution_results": []any{map[string]any{"tool": "fs.list", "summary": "listed 1 entries in ."}}}}
	...
	t.Run("uses responder output", func(t *testing.T) {
		...
		if !strings.Contains(seen.ToolSummary, "fs.list") {
