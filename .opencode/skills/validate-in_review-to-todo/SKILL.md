# Skill: validate-in_review-to-todo

Validates the FSM transition `in_review → todo` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: in_review`.
2. Confirm `todo` is a valid next state from `in_review`.
