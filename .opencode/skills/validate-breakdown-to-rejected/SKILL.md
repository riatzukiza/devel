# Skill: validate-breakdown-to-rejected

Validates the FSM transition `breakdown → rejected` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: breakdown`.
2. Confirm `rejected` is a valid next state from `breakdown`.
