# Skill: validate-accepted-to-icebox

Validates the FSM transition `accepted → icebox` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: accepted`.
2. Confirm `icebox` is a valid next state from `accepted`.
