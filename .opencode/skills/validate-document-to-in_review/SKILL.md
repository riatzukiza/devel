...
Validates the FSM transition `document → in_review` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: document`.
2. Confirm `in_review` is a valid next state from `document`.
