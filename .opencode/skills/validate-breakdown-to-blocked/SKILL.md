# Skill: validate-breakdown-to-blocked

Validates the FSM transition `breakdown → blocked` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: breakdown`.
2. Confirm `blocked` is a valid next state from `breakdown`.
...
4. If `blocked` has WIP caps, verify capacity (via kanban CLI / configured limits).
