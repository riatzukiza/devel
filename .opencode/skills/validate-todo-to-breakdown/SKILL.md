# Skill: validate-todo-to-breakdown

Validates the FSM transition `todo → breakdown` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: todo`.
2. Confirm `breakdown` is a valid next state from `todo`.
