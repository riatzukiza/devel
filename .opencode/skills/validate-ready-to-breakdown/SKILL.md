# Skill: validate-ready-to-breakdown

Validates the FSM transition `ready → breakdown` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: ready`.
2. Confirm `breakdown` is a valid next state from `ready`.
