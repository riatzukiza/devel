# Skill: validate-in_review-to-in_progress

Validates the FSM transition `in_review → in_progress` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: in_review`.
2. Confirm `in_progress` is a valid next state from `in_review`.
