# Skill: validate-testing-to-in_review

Validates the FSM transition `testing → in_review` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: testing`.
2. Confirm `in_review` is a valid next state from `testing`.
