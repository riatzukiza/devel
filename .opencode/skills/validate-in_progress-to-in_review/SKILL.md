# Skill: validate-in_progress-to-in_review

Validates the FSM transition `in_progress → in_review` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: in_progress`.
2. Confirm `in_review` is a valid next state from `in_progress`.
...
4. If `in_review` has WIP caps, verify capacity (via kanban CLI / configured limits).
