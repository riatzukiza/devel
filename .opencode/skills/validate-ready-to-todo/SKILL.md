# Skill: validate-ready-to-todo

Validates the FSM transition `ready → todo` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: ready`.
2. Confirm `todo` is a valid next state from `ready`.
