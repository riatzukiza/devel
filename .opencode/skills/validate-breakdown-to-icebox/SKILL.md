# Skill: validate-breakdown-to-icebox

Validates the FSM transition `breakdown → icebox` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: breakdown`.
2. Confirm `icebox` is a valid next state from `breakdown`.
...
4. If `icebox` has WIP caps, verify capacity (via kanban CLI / configured limits).
