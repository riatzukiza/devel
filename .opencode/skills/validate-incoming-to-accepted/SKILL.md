# Skill: validate-incoming-to-accepted

Validates the FSM transition `incoming → accepted` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: incoming`.
2. Confirm `accepted` is a valid next state from `incoming`.
...
4. If `accepted` has WIP caps, verify capacity (via kanban CLI / configured limits).
