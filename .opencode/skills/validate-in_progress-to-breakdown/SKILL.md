# Skill: validate-in_progress-to-breakdown

Validates the FSM transition `in_progress → breakdown` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: in_progress`.
2. Confirm `breakdown` is a valid next state from `in_progress`.
