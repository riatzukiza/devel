# Skill: validate-breakdown-to-ready

Validates the FSM transition `breakdown → ready` using `docs/reference/process.md` rules and WIP checks.

## Steps
1. Confirm task currently has `status: breakdown`.
2. Confirm `ready` is a valid next state from `breakdown`.
...
4. If `ready` has WIP caps, verify capacity (via kanban CLI / configured limits).
