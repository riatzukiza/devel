...
async function main(): Promise<void> {
  ...
  await eventBus.subscribe(
    ...
    "cephalon",
    async (event) => {
      const payloadPreview = safeStringify(event.payload).slice(0, 100);
  ...
  const gracefulShutdown = async (signal: string): Promise<void> => {
    ...
    try {
      await memoryUI.stop();
      await discord.stop();
      console.log("[Shutdown] Discord disconnected");
    } catch (err) {
