...
} from "../prompts/duck-persona.js";
import { chunkMessage } from "../utils/message-chunk.js";
...
const TOOL_REGISTRY: Record<string, ToolRegistryEntry> = {
  ...
  "discord.send": {
    schema: {
      name: "discord.send",
      description:
        "Send a message to a Discord channel from discord.list.channels. Long messages will be automatically split.",
      parameters: {
        type: "object",
        properties: {
          channel_id: {
          ...
          text: {
            type: "string",
            description: "The message text to send (will be split if > 4000 chars)",
          },
          reply_to: {
            type: "string",
    ...
    handler: async (args, deps) => {
      ...
      try {
        ...
        if (chunks.length === 1) {
          ...
          // Multiple chunks, send each one
          console.log(`[discord.send] Splitting message into ${chunks.length} chunks`);
          ...
          for (let i = 0; i < chunks.length; i++) {
            ...
            const replyTo = i === 0 ? reply_to : messageIds[i - 1];
            
            const chunkText = isLast 
              ? `${chunkNum}/${chunks.length} 🦆 ${chunks[i]}`
              : `${chunkNum}/${chunks.length} 🦆 ${chunks[i]} (cont...)`;
...
export class TurnProcessor {
  ...
  async processTurn(session: Session, event: CephalonEvent): Promise<void> {
    ...
    try {
      ...
      // For tick events, add cycling entertainment prompt instead of static one
      if (event.type === "system.tick") {
        ...
        const tickNumber = tickPayload.tickNumber || 0;
        const promptIndex = tickNumber % 7;

        // Get cycling entertainment prompt
        const cyclingPrompt = getCyclingPrompt(tickNumber);
        const promptName = getPromptName(promptIndex);
        const formattedRecentActivity = formatRecentActivityForPrompt(
          tickPayload.recentActivity,
        );

        console.log(
          `[TurnProcessor] Tick #${tickNumber} - Using prompt: ${promptName}`,
        );

        messages.push({
          role: "user" as const,
          content: cyclingPrompt.replace(
            "{recentActivity}",
