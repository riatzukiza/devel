...
test("forwards openplanner request through facade route", async () => {
  ...
  try {
    ...
    assert.equal(res.statusCode, 200);
    const body = res.json() as { ok: boolean; rows: Array<{ session: string }> };
...
test("returns cors headers and handles preflight", async () => {
  const app = await buildApp({
    host: "127.0.0.1",
    port: 0,
    openplannerUrl: "http://127.0.0.1:7777",
...
test("lists workspace entries and supports read/write", async () => {
  ...
  try {
    ...
    assert.ok(listBody.entries.some((entry) => entry.path === "README.md" && entry.type === "file"));
    ...
    const readBody = readResponse.json() as { ok: boolean; content: string };
    ...
    const verifyBody = verifyResponse.json() as { content: string };
