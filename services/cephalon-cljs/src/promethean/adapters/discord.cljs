...
(defn update-presence!
  "Update bot's presence/activity on Discord"
  [client status-text]
  (if (and client (.-user client) status-text (not= "" status-text))
    (.setActivity (.-user client) status-text)
    (js/Promise.reject (js/Error. "discord client not ready or empty status"))))

(defn update-user-profile!
  "Update bot's profile (username, avatar, display name)"
  [client {:keys [username avatar-url display-name]}]
  (if (and client (.-user client))
    (let [user (.-user client)
          actions (cond-> []
                    (and username (not= "" username))
                    (conj (.setUsername user username))
                    (and avatar-url (not= "" avatar-url))
                    (conj (.setAvatar user avatar-url))
                    (and display-name (not= "" display-name))
                    (conj (.setDisplayName user display-name)))]
      (if (seq actions)
        (js/Promise.all (clj->js actions))
        (js/Promise.reject (js/Error. "no profile fields provided"))))
    (js/Promise.reject (js/Error. "discord client not ready"))))
