...
void test("routes /api/mcp/:service/* through mcp mux and preserves mcp-session-id", async () => {
  const upstream = Fastify();
  upstream.post("/mcp", async (_req, reply) => {
    await reply
      .header("content-type", "application/json")
      .header("mcp-session-id", "ses_mux_123")
      .send({ ok: true, backend: "files-mux" });
  });
  await upstream.listen({ host: "127.0.0.1", port: 0 });
  ...
  const app = await createApp({
    ...
    workspaceRoot: "/tmp",
    mcpUrl: "http://127.0.0.1:3001",
    mcpServiceUrls: { files: `http://127.0.0.1:${address.port}` },
    oauthEnabled: false,
    oauthIssuer: "http://localhost:3001",
    oauthAudience: "api-gateway",
    allowedHosts: ["localhost", "127.0.0.1"]
  ...
  try {
    ...
    assert.equal(res.statusCode, 200);
    assert.equal(res.headers["mcp-session-id"], "ses_mux_123");
    assert.deepEqual(res.json(), { ok: true, backend: "files-mux" });
  } finally {
    await app.close();
    await upstream.close();
  }
