...
* **Write ecosystem configs in CLJS-ish DSL files** (readable, composable, versionable).
...
* **Legacy:** `pm2-clj` was a pass-through to PM2, translating DSL → a temp `ecosystem.config.cjs` and then executing PM2.
* **Legacy:** `clobber` previously wrapped `pm2-clj`.
...
* If it sees a DSL ecosystem file (`.pm2.clj`, `.pm2.cljs`, `.pm2.edn`, or plain `.clj/.cljs/.edn`), it:
...
### `ecosystems/stacks/all.pm2.clj` (composition via include)

```clojure
(pm2-clj.dsl/compose
  (pm2-clj.dsl/include "./base.pm2.clj")
  (pm2-clj.dsl/include "../ecosystems/services/api.pm2.clj")
  (pm2-clj.dsl/include "../ecosystems/services/worker.pm2.clj"))
```

Run:

```bash
pm2-clj start ecosystems/stacks/all.pm2.clj --mode prod
```
...
```bash
pm2-clj start ecosystems/base.pm2.clj --mode dev
pm2-clj start ecosystems/base.pm2.clj --mode prod
```
...
```bash
pm2-clj render ecosystems/base.pm2.clj --mode prod
```

### Use clobber (currently delegated)

```bash
clobber pm2 start ecosystems/base.pm2.clj --mode dev
```
