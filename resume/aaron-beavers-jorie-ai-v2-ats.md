...
- **7-stage generative prompt evaluation dataset pipeline** (Clojure + Python bridge): deterministic, provenance-rich dataset builds with leakage-proof splits (sentence-transformers + HDBSCAN) and reproducibility bundles (Parquet + SHA-256 manifests + datasheets).  
...
*Jul 2020 – Sep 2023*
- Built and maintained a **5-stage Celery-based automation pipeline** for uploaded-file processing: parse → validate → summarize → generate error reports → notify stakeholders; designed for safe failure modes and operational visibility.
